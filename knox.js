function listStatus()
{
  ...
  //     req.channel.redirectionLimit = 0;
...
function cbWrapper(req, funct){
  index = req.responseText.indexOf("\<html\>", req.body);
  if(index > 0)
    // window.location.replace("/knoxauth/login.html?originalUrl=https://www.local.com:8443/knoxplorer");
    // window.location.reload();
    top.location.href=req.responseURL;//redirection
  else
    funct(req.responseText);
}
...
function getWebHDFSPath() {
  ...
  if (topo === "") {
    webhdfs = "c6401.ambari.apache.org:8443/gateway/sandbox-apps/webhdfs/v1";
  }
  else {
